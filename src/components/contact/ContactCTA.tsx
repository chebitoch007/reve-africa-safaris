 ...
 * ContactCTA
 *
 * Contact page final CTA. Client Component — scroll-reveal.
 * Surface: bg-inverse (basalt-900).
 *
 * Uses bg-inverse rather than bg-deep to distinguish from the hero
 * (which is bg-deep) and avoid deep→…→deep bookending. Warm amber glow
 * and dual CTA row consistent with all prior page CTAs.
 *
 * Both buttons are anchor links: primary scrolls to #contact-form,
 * secondary is a tel: link for immediate phone contact.
 */
...
import { CONTACT_CTA } from '@/lib/constants/contact';
...
export function ContactCTA() {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden py-28 md:py-40 bg-[var(--color-bg-inverse)]"
    >
      {/* Amber glow */}
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse at 50% 80%, rgba(212,160,23,0.10) 0%, transparent 60%)',
        }}
        aria-hidden="true"
      />
      ...
      <div className="relative z-[1] w-full mx-auto max-w-[90rem] px-5 sm:px-8 lg:px-16 text-center">
        ...
        >
          ...
          <Link
            href={CONTACT_CTA.secondaryCTA.href}
            className={cn(
              'inline-flex items-center justify-center',
              'px-10 py-4 min-w-[11rem]',
