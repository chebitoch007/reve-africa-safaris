 ...
 * PackagesCTA
 *
 * Safari Packages page closing call to action. Client Component.
 * Surface: bg-deep (basalt-950) — consistent with all other page CTAs.
...
import { PACKAGES_CTA } from '@/lib/constants/packages';
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: 'radial-gradient(ellipse at 50% 80%, rgba(212,160,23,0.08) 0%, transparent 62%)' }}
        aria-hidden="true"
      />
      ...
      <div className="relative z-[1] w-full mx-auto max-w-[90rem] px-5 sm:px-8 lg:px-16 text-center">
        ...
        >
          <Link
            href={PACKAGES_CTA.primaryCTA.href}
            className={cn(
          ...
          >
            {PACKAGES_CTA.primaryCTA.label}
          </Link>

          <Link
            href={PACKAGES_CTA.secondaryCTA.href}
            className={cn(
              'inline-flex items-center justify-center',
              'px-10 py-4 min-w-[11rem]',
          ...
          >
            {PACKAGES_CTA.secondaryCTA.label}
          </Link>
        </motion.div>
